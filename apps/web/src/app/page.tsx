    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-6 py-16 md:grid-cols-2 md:items-center">
        <div>
          ...
          <p className="mt-5 max-w-xl text-lg text-slate-600">
            Curated gear for home, work, and everyday movement. Built for a fast, modern shopping experience.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            <Link href="/products" className="rounded-full bg-slate-900 px-6 py-3 font-medium text-white">
              Shop now
            </Link>
            <Link href="/inventory" className="rounded-full border border-slate-300 bg-white px-6 py-3 font-medium text-slate-800">
              View inventory
            </Link>
          </div>
        ...
        <div className="relative">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl">
            <div className="rounded-[1.5rem] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 p-6 text-white">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-300">Featured collection</p>
      ...
      <section id="shop" className="mx-auto max-w-6xl px-6 pb-16">
        <div className="mb-8 flex items-end justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-700">Trending</p>
            <h2 className="mt-2 text-3xl font-bold">Popular products</h2>
          </div>
          <Link href="/products" className="text-sm font-medium text-slate-700">See all products</Link>
        </div>
